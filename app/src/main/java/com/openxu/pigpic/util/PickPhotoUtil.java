...
//				}
				//去掉针对三星手机的
				//如果在没有设置额外的控制图片存储路径的参数MediaStore.EXTRA_OUTPUT情况下，
				// 返回的结果将是一个小的Bitmap，这仅仅只适合于应用程序需要一张小的图片的时候；
				// 如果设置了MediaStore.EXTRA_OUTPUT，那么将保存整个大小的到指定的Uri中
	 ...
	 * @param mActivity
	 * @param uri
	 * @param outputX 裁剪图片的宽，如果传递的数值<=0，则赋默认值80
	 * @param outputY 裁剪图片的高，如果传递的数值<=0，则赋默认值80
	 */
	...
	/**
	 * 拍照时图片的数据库操作<BR>
	 * 主动调用主要是防止拍照是，系统没有主动保存数据库，回调查询该照片不为空
	 * 
	 * @param mContext
	 *            当前对象
	 * @param obj
